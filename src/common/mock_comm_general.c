...
int tcpConnectWrapper([[maybe_unused]] int sockFd, char* ipAddress,
  ...
  if (!mockPairs[port].in_use) {
    // fprintf(stderr, "tcpConnectWrapper: Socket pair for port %d has not been
    // created.\n", port);
    return -1;
  }
...
int tcpAcceptWrapper(int sockFd) {
  // Loop through the socket pairs to find which one matches the given server
  // fd.
  ...
  // fprintf(stderr, "tcpAcceptWrapper: Provided sockFd %d not found in any
  // socket pair.\n", sockFd);
  return -1;
}
