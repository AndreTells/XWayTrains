 ...
 */
int serializeResourceRequest(const ResourceRequest_t *req, unsigned char *buffer) {
    if (!req || !buffer) {
        return -1;
    }
    size_t offset = 0;
 ...
 * @return int 0 on success, negative on error.
 */
int deserializeResourceRequest(const unsigned char *buffer, ResourceRequest_t *req) {
    if (!req || !buffer) {
        return -1;
    }
    size_t offset = 0;
 ...
 * @return int 0 on success, negative on error.
 */
int serializeResourceRequestResponse(const ResourceRequestResponse_t *resp, unsigned char *buffer) {
    if (!resp || !buffer) {
        return -1;
    }
    size_t offset = 0;
 ...
 * @return int 0 on success, negative on error.
 */
int deserializeResourceRequestResponse(const unsigned char *buffer, ResourceRequestResponse_t *resp) {
    if (!resp || !buffer) {
        return -1;
    }
    size_t offset = 0;
...
int sendResourceRequest(int fd,ResourceRequest_t* req){
  verbose("[RESOURCE REQUEST]: sending request from %d request type %d for resource %d through %d\n",
...
int answerResourceRequest(int fd, ResourceRequestResponse_t* resp){
  verbose("[RESOURCE REQUEST]: sending response to %d resp type %d for resource %d through %d\n",
...
ResourceRequestResponse_t* recvResourceRequestResponse(int fd){
  ...
  res = read(fd, buf, RESOURCE_REQUEST_RESPONSE_SERIALIZED_SIZE);// MAY CAUSE AN ISSUE
