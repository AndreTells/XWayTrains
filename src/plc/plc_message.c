...
typedef enum { MODBUS_ETHWAY } ModuBusId_e;
...
PlcMessage_t* createACK(PlcMessage_t* msg, bool res) {
  ...
  setNPDU(ack, msg->npdu.type, msg->npdu.receiver, msg->npdu.sender, ext);
...
PlcMessage_t* deserializePlcMessage(uint8_t* serMsg) {
  ...
  if (apduCode == APDU_WRITE_REQ) {
    ...
    memcpy(&msgDataLen, serMsg + 5,
           sizeof(uint16_t));  // implicitly inverts the bit order
    len += msgDataLen * 2;
