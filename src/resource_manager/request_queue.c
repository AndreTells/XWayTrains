...
int destroyQueue(ResourceRequestQueue_t* queue) {
  ...
  while (queue->head) {
    ...
    ResourceRequestResponse_t* resp =
        createResourceRequestResponse(req, RESOURCE_REFUSED);
...
int pushQueue(ResourceRequestQueue_t* queue, ResourceRequest_t* req) {
  ...
  ResourceRequestQueueEntry_t* newEntry =
      (ResourceRequestQueueEntry_t*)malloc(sizeof(ResourceRequestQueueEntry_t));
  if (newEntry == NULL) {
    return -1;
  }
