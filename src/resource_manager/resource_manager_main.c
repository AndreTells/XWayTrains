...
int main(int argc, char* argv[]) {
  ...
  if (safeDatabase == NULL) {
    verbose("[RESOURCE_MANAGER_MAIN]: Database ... " VERBOSE_KRED
            "fail \n" VERBOSE_RESET);
    return -1;
  }
  verbose("[RESOURCE_MANAGER_MAIN]: Database ... " VERBOSE_KGRN
          "success \n" VERBOSE_RESET);
  ...
  if (manager == NULL) {
    verbose(
        "[RESOURCE_MANAGER_MAIN]: Initializing Resource Manager "
        "... " VERBOSE_KRED "fail \n" VERBOSE_RESET);
    return -1;
  }
  verbose(
      "[RESOURCE_MANAGER_MAIN]: Initializing Resource Manager ... " VERBOSE_KGRN
      "success \n" VERBOSE_RESET);

  while (true) {
    verbose(
        "[RESOURCE_MANAGER_MAIN]: Waiting for a Train Manager Connection ... "
        "\n");
